...
if __name__ == "__main__":
    ...
    for i, ax in enumerate(axs.flat):
        ...
        for k in kart:
            ax.add_patch(patches.Rectangle((k[0] - 0.5, k[1] - 0.5), k[2] - k[0], k[3] - k[1], fc="none", ec="r", lw=2))
        for k in bomb:
            ax.add_patch(patches.Rectangle((k[0] - 0.5, k[1] - 0.5), k[2] - k[0], k[3] - k[1], fc="none", ec="g", lw=2))
        for k in pickup:
            ax.add_patch(patches.Rectangle((k[0] - 0.5, k[1] - 0.5), k[2] - k[0], k[3] - k[1], fc="none", ec="b", lw=2))
        ax.axis("off")
    dataset = DetectionSuperTuxDataset(
        "dense_data/train",
        transform=dense_transforms.Compose([dense_transforms.RandomHorizontalFlip(0), dense_transforms.ToTensor()]),
    )
    fig.tight_layout()
